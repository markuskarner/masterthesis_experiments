...
model_name = 'improved_VGG16_one_pixel_cifar10_trained_model.h5'
...
def _define_model():
    ...
    _model.add(AveragePooling2D(2, 2))
...
callback = keras.callbacks.EarlyStopping(monitor='val_loss', patience=30, verbose=1)
...
plt.savefig(os.path.join('model_history', 'improved_VGG16_one_pixel_CIFAR10_model_accuracy.png'))
plt.clf()  # clear plot
...
plt.xlabel('epoch')
plt.legend(['train', 'test'], loc='upper left')
plt.savefig(os.path.join('model_history', 'improved_VGG16_one_pixel_CIFAR10_model_loss.png'))
